...
class Program
{
    static void Main()
    {
        ...
        Console.WriteLine("Número de plataformas: " + platforms.Length);
        ...
        IMem<float> bufferA = Cl.CreateBuffer<float>(context, MemFlags.ReadOnly | MemFlags.CopyHostPtr, A, out error);

        IMem<float> bufferB = Cl.CreateBuffer<float>(context, MemFlags.ReadOnly | MemFlags.CopyHostPtr, B, out error);

        IMem<float> bufferC = Cl.CreateBuffer<float>(context, MemFlags.WriteOnly, N, out error);
        ...
        Cl.EnqueueNDRangeKernel(queue, kernel, 1, null, workSize, null, 0, null, out _);

        Cl.Finish(queue);

        Cl.EnqueueReadBuffer(queue, bufferC, Bool.True, IntPtr.Zero, new IntPtr(N * sizeof(float)), C, 0, null, out _);
